...
final public class ByteConvert {
	public final static String lineSeperate = System
	...
	public static int byte2int2(byte[] convertByte, int offset,
		...
		byte0 = convertByte[0 + offset] < 0
				? convertByte[0 + offset] + 256
				: convertByte[0 + offset];
		byte1 = convertByte[1 + offset] < 0
	...
	public static int byte2int4(byte[] convertByte, int offset,
		...
		byte1 = convertByte[1 + offset] < 0
				? convertByte[1 + offset] + 256
				: convertByte[1 + offset];
		byte2 = convertByte[2 + offset] < 0
				? convertByte[2 + offset] + 256
				: convertByte[2 + offset];
		byte3 = convertByte[3 + offset] < 0
				? convertByte[3 + offset] + 256
				: convertByte[3 + offset];
		if (!bigIndian)
			value = (byte3 << 24) + (byte2 << 16) + (byte1 << 8) + byte0;
	...
	public static byte[] byteString2Bytes(String s) {
		...
		if (ilen <= 0) {
			...
			while (st.hasMoreElements()) {
				// GeneralLogger.finest("!!!!!!!!!!!!!!in function string2Bytes
				// while(st.hasMoreElements()");
				// buf[i++]=Byte.parseByte(st.nextToken(),16);
				try {
					buf[i] = (byte) ((Integer.parseInt(st.nextToken(), 16))
							& 0xff);
				} catch (NumberFormatException ex) {
					System.out.println(
							"Exception in string2Bytes : " + ex.getMessage());
					buf[i] = (byte) 0xff;
				}
				i++;
			}
	 ...
	 * 
	 */
	public static String bytesToHexString(byte[] src, boolean isUpperCase) {
	...
	public static String bytesToHexString(byte[] src, int length) {
		StringBuilder stringBuilder = new StringBuilder("");
		if (src == null || src.length <= 0 || length > src.length || length < 0) {
			return null;
		}
	...
	public static byte[] hexStringsToByte(String strInput){
		byte [] result = new byte[strInput.length()/2];
		for(int i=0;i<result.length;i++){
			int start = i * 2;
			int end = start +2;
			String tmp = strInput.substring(start, end);
			result[i] = hexStringToByte(tmp.getBytes());
		}
		return result;
	}
	private static byte hexStringToByte(byte[] byteArry) {
		for (int i = 0; i < 2; i++) {
